...
用户明确确认修改范围后，Agent 进入执行锁定状态：

1. **只改确认清单内的文件或产物**：不得借“顺手优化”“统一重构”“顺便修复”扩大范围。
2. **只做确认过的改动**：不得擅自改变规则口径、registry 元数据、execution_ledger 结构、report JSON 结构、UI 展示口径或 LLM 调用逻辑。
3. **范围变化必须停手再确认**：执行中若发现必须修改范围外文件，先停止并说明“为什么必须改、不改的后果、新增范围、验收方式”，等待用户确认后再继续。
4. **测试也受范围约束**：默认只建议或执行与本次改动直接相关的小测试；完整测试、生成报告、生成标注副本均需符合已确认范围。
5. **完成后按事实回报**：列明实际修改文件、实际改动、未修改内容、建议验收方式和剩余风险。
...
- 每次回复最后必须以 `公主请审阅~` 结尾，作为协作规则是否被执行的可见标记。
