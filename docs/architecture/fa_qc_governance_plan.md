...
上述能力的当前状态为循环B候选基线。真实样本复测5张，其中4张 findings 已恢复至合理水平；Run 62仍存在较多资产编号唯一性 findings，用户已人工确认该问题属实。该样本后续不作为唯一性误报修复依据，而作为同类批量真实问题的展示颗粒度优化样本。

### 3.3 同类批量 findings 的展示颗粒度

同一质检点下确实存在大量同质异常时，不应把每一行证据都默认展示为独立管理层问题。系统应区分三层：

- 问题簇：面向 reviewer、manager 和汇报，默认只展示一项审计事项，例如“资产编号唯一性问题1项”。
- 异常组：面向 preparer 整改，按重复编号、缺失字段、异常类别或样本问题分组。
- 证据行：面向系统追溯和底稿定位，保留原始行号、单元格、资产编号和取数依据。

可优先接入该机制的规则包括：`unique_asset_id`、`fa_list_required_fields`、`addition_required_fields`、`disposal_required_fields`、`useful_life_positive`、`salvage_rate_range`。金额差异、样本匹配、政策例外等规则可在第二批扩展。

聚合不得改变底层规则事实，不得为减少 findings 数量而删除证据。JSON 应保留原始 findings 或可追溯明细；UI/HTML 默认展示问题簇并支持展开异常组；标注底稿可使用摘要批注加轻量高亮，避免逐单元格长批注造成审计资源浪费。

聚合必须满足：同一规则、同一工作表或业务作用域、同一问题原因、同一严重程度、同一整改动作。涉及重大金额、不同法人主体、不同审计认定、不同根因或需要单独审计判断的异常，不得强行合并。
