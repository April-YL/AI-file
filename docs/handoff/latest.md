...
**LLM 定位补充**：LLM 不应被简化为“报告后的兜底复核”。合理职责包括识别层兜底，例如表头/字段映射、说明区识别、样本描述和复杂语义块识别；也可用于语义类质检点辅助判断，例如拒绝理由、波动说明、证据充分性、checklist 满足度。边界不变：金额勾稽、唯一性、必填、清单合计等确定性事项由 `rules` 优先判断；LLM 不得覆盖确定性 `FAIL`，也不得自行扩大 SOP/checklist 口径。
