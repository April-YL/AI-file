...
- 本轮纳入 FA list 是为修复已确认的批量误报和检查覆盖漏失，不改变 K.00–K.03 的长期主线。正式决策见 [`ADR-0003`](../decisions/ADR-0003-unified-orchestrator-and-llm-governance.md)，规则治理见 [`fa_qc_governance_plan.md`](../architecture/fa_qc_governance_plan.md)。当前仅完成目标沉淀，尚未实施相应代码。
...
| LLM | 客户端、配置、脱敏及多个可选调用点 | 尚未形成统一 Router、分能力开关、按规则控制和一致失败策略；默认关闭 |
