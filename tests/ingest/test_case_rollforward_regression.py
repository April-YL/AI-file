"""案例库 K.01 识别层批量回归（需本地 ``固定资产质检agent/案例库``）。"""

from __future__ import annotations

import json
from pathlib import Path

import pytest

from ingest.case_library import (
    DEFAULT_MAX_WORKBOOK_MB,
    find_case_library_dir,
    iter_case_workbooks,
    should_skip_case_workbook,
)
from ingest.rollforward_sheet import K01_SECTION_IDS, load_rollforward_from_workbook
from rules.registry import attach_rule_metadata
from rules.rollforward_runner import run_rollforward_rules

_ROOT = Path(__file__).resolve().parents[2]
_ARTIFACT = _ROOT / "artifacts" / "case_rollforward_regression.json"
_REGRESSION_MAX_ROWS = 150
_EXPECTED_PROFILE = "hybrid"
_EXPECTED_SECTIONS = 6


def _case_dir() -> Path | None:
    return find_case_library_dir(_ROOT)


@pytest.fixture(scope="module")
def case_refs():
    case_dir = _case_dir()
    if case_dir is None:
        pytest.skip("案例库目录不存在")
    return iter_case_workbooks(_ROOT, max_mb=DEFAULT_MAX_WORKBOOK_MB)


def test_a_company_workbook_always_skipped(case_refs):
    a_refs = [r for r in case_refs if "A有限" in r.path.name or "A公司" in r.path.name]
    if not a_refs:
        pytest.skip("案例库中无 A 公司底稿")
    for ref in a_refs:
        assert ref.skipped
        assert ref.skip_reason is not None


@pytest.mark.parametrize("label", ["B", "C", "D", "E", "F", "G"])
def test_case_library_k01_recognition_baseline(label: str, case_refs):
    """B–G：hybrid + 六区块 + 置信度 + 期末合计可读。"""
    targets = [r for r in case_refs if not r.skipped and label in r.path.stem]
    if not targets:
        pytest.skip(f"未找到案例 {label}")
    ref = targets[0]
    assert should_skip_case_workbook(ref.path) is None

    rf = load_rollforward_from_workbook(ref.path, max_rows=_REGRESSION_MAX_ROWS)
    assert rf.source_sheet
    assert "K.01" in rf.source_sheet or "Agree" in rf.source_sheet
    assert rf.layout_profile.value == _EXPECTED_PROFILE

    present = sum(1 for sid in K01_SECTION_IDS if rf.section_presence.get(sid))
    assert present == _EXPECTED_SECTIONS, rf.section_presence
    assert rf.ending_totals.get("net_value") is not None or rf.ending_totals.get("original_value") is not None


@pytest.mark.parametrize("label", ["B", "C", "D", "E", "F", "G"])
def test_case_library_k01_p0_rules_pass(label: str, case_refs):
    """B–G：P0 规则 rollforward_exists + rollforward_columns_complete 无 FAIL。"""
    targets = [r for r in case_refs if not r.skipped and label in r.path.stem]
    if not targets:
        pytest.skip(f"未找到案例 {label}")
    ref = targets[0]
    rf = load_rollforward_from_workbook(ref.path, max_rows=_REGRESSION_MAX_ROWS)
    issues = attach_rule_metadata(run_rollforward_rules(rf))
    fails = [
        i
        for i in issues
        if i.rule_id
        in (
            "rollforward_exists",
            "rollforward_columns_complete",
            "rollforward_abnormal_amounts",
        )
    ]
    assert not fails, [(i.rule_id, i.message) for i in fails]


def test_case_library_k01_loads_without_error(case_refs):
    runnable = [r for r in case_refs if not r.skipped]
    if not runnable:
        pytest.skip("无可跑案例库底稿")
    for ref in runnable:
        rf = load_rollforward_from_workbook(ref.path, max_rows=_REGRESSION_MAX_ROWS)
        assert rf.source_sheet


@pytest.mark.skipif(not _ARTIFACT.is_file(), reason="先运行 scripts/run_case_rollforward_regression.py")
def test_regression_artifact_matches_live_run(case_refs):
    """JSON 回归表与现场跑批一致（防漂移）。"""
    saved = json.loads(_ARTIFACT.read_text(encoding="utf-8"))
    by_file = {r["file"]: r for r in saved["rows"]}
    for ref in case_refs:
        if ref.skipped:
            continue
        rf = load_rollforward_from_workbook(ref.path, max_rows=_REGRESSION_MAX_ROWS)
        live = {
            "rollforward_sheet": rf.source_sheet,
            "layout_profile": rf.layout_profile.value,
            "sections_detected": sum(1 for sid in K01_SECTION_IDS if rf.section_presence.get(sid)),
            "recognition_confidence": rf.recognition_confidence,
            "section_presence": dict(rf.section_presence),
        }
        snap = by_file.get(ref.path.name)
        assert snap is not None, ref.path.name
        assert snap["status"] == "ok"
        for k, v in live.items():
            assert snap[k] == v, f"{ref.path.name}.{k}: snap={snap[k]!r} live={v!r}"
